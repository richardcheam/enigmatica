...
- The clue required to extract `TRAP` is `14 34 24 96`.
...
The file is not in the repo yet, so this image is intentionally not embedded yet.
...
- What `mod 31` means:
  it means "take the remainder after division by 31" so every value is wrapped back into the range `0..30`.
  Example: `138 mod 31 = 14` because `138 = 31 * 4 + 14`.
- An unrelated worked example with the same inverse matrix is `LB -> OK`:
  `L = 11`, `B = 1`, so begin with `[11, 1]`.
- Apply the inverse matrix modulo 31:
  `[12 6; 12 2] * [11; 1] = [138; 134] = [14; 10] mod 31`.
- Convert `[14, 10]` back into letters:
  `14 -> O`, `10 -> K`.
- Read the ciphertext as four blocks: `QB RA EO ME`
- Repeat that calculation for each ciphertext block.
